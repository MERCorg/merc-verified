...
quotient as the partition, any τ*-path between same-block states admits an inert τ*-path
(every intermediate state has the same block as the endpoints).

Proof strategy: generalize the relation over an explicit block `B`, then do well-founded
induction on `(b, a)` with `Prod.Lex` of `TransGen hWF` on both coordinates. The lex order
admits simultaneous descent (multi-step τ-jumps in either coordinate via one IH application).
After splitting the path's head step `a →τ→ a₁ →τ*→ b`, forward-match `a →τ→ a₁` along the
bisimulation `r`:
* `Or.inl` (inert): `r a₁ b`. Head step is inert; descend in the start coordinate.
* `Or.inr` with `⟦t'⟧ = B`: matched step is inert. Same closure as `Or.inl`.
* `Or.inr` with `⟦t'⟧ ≠ B`: matched step `t →τ→ t'` is non-inert, so `⟦a₁⟧ = ⟦t'⟧ ≠ B`.
  The remaining `sorry` lives here — see the in-line note for the obstacle. This is the
  classical "stuttering" property (van Glabbeek–Weijland 1996, Basten 1996); a complete
  formalisation requires bridging the BB structure to construct an inert path of `a →τ*→ b`
  that may diverge from the original path. -/
private theorem path_inertness [Cslib.HasTau Label] {lts : Cslib.LTS State Label}
    ...
    ∀ {x y : State}, lts.τSTr x y →
      ...
      Relation.ReflTransGen
        ...
        x y := by
  -- Strategy: generalize the relation over an explicit block parameter `B` and prove the result
  -- by well-founded induction on the pair `(b, a)` using `Prod.Lex` of `TransGen hWF` on both
  -- coordinates. The lexicographic order admits two descent shapes simultaneously:
  --   • Endpoint-descent: b → b' via TransGen (so the path endpoint moves τ-strictly down).
  --   • Equal-endpoint start-descent: b unchanged, a → a' via TransGen (start moves down).
  -- TransGen on both lets a single IH absorb multi-step jumps generated by BB matching.
  suffices main : ∀ (B : Quotient (brSetoid lts)) (a b : State),
      lts.τSTr a b → Quotient.mk (brSetoid lts) a = B →
      Quotient.mk (brSetoid lts) b = B →
      Relation.ReflTransGen
        (fun u v => lts.Tr u Cslib.HasTau.τ v ∧
          Quotient.mk (brSetoid lts) u = B ∧
          Quotient.mk (brSetoid lts) v = B) a b by
    intro x y hPath hπ
    -- Use B := ⟦x⟧, and reuse the generalized lemma's conclusion as-is.
    exact main (Quotient.mk (brSetoid lts) x) x y hPath rfl hπ.symm
  intro B
  -- Local abbreviation for the indexed relation; we keep it as a `set` declared inside the
  -- WF-induction body so that `R` is in scope for the cases.
  -- Curried statement: take a, b explicitly so cases avoid pair projections.
  suffices aux : ∀ (b a : State),
      lts.τSTr a b →
      Quotient.mk (brSetoid lts) a = B →
      Quotient.mk (brSetoid lts) b = B →
      Relation.ReflTransGen
        (fun u v => lts.Tr u Cslib.HasTau.τ v ∧
          Quotient.mk (brSetoid lts) u = B ∧
          Quotient.mk (brSetoid lts) v = B) a b by
    intro a b hPath hπa hπb
    exact aux b a hPath hπa hπb
  -- WF induction on (b, a) with an explicit motive that takes b and a as separate arguments.
  intro b a
  refine WellFounded.induction
      (WellFounded.prod_lex (WellFounded.transGen hWF) (WellFounded.transGen hWF))
      (C := fun p => ∀ (b a : State), p = (b, a) →
        lts.τSTr a b →
        Quotient.mk (brSetoid lts) a = B →
        Quotient.mk (brSetoid lts) b = B →
        Relation.ReflTransGen
          (fun u v => lts.Tr u Cslib.HasTau.τ v ∧
            Quotient.mk (brSetoid lts) u = B ∧
            Quotient.mk (brSetoid lts) v = B) a b)
      (b, a) ?_ b a rfl
  rintro ⟨b, a⟩ ih b' a' hEq hPath hπa hπb
  rcases hEq with ⟨rfl, rfl⟩
  -- Local notation for the indexed relation.
  set R : State → State → Prop := fun u v =>
      lts.Tr u Cslib.HasTau.τ v ∧
        Quotient.mk (brSetoid lts) u = B ∧
        Quotient.mk (brSetoid lts) v = B with hR
  -- Split the path: empty | head step `a →τ→ a₁ →τ*→ b`.
  rcases Relation.ReflTransGen.cases_head hPath with hEq | ⟨a₁, hFirst, hTail⟩
  · subst hEq; exact Relation.ReflTransGen.refl
  -- Extract a bisimulation `r` witnessing `⟦a⟧ = ⟦b⟧`.
  obtain ⟨r, hrab, hBis⟩ := Quotient.exact (hπa.trans hπb.symm)
  -- Helper: two r-related pairs to a common state give partition equality.
  ...
  -- Forward-match the head step `a →τ→ a₁` along the bisimulation `r`.
  rcases (hBis hrab Cslib.HasTau.τ).1 a₁ hFirst with
    ⟨-, hra₁b⟩ | ⟨t, t', hSTr, hTrt, hrat, hra₁t'⟩
  · -- CASE A (Or.inl): `r a₁ b`, so ⟦a₁⟧ = ⟦b⟧ = B.
    have hπa₁ : Quotient.mk (brSetoid lts) a₁ = B :=
      (Quotient.sound (s := brSetoid lts) ⟨r, hra₁b, hBis⟩).trans hπb
    -- Lex-smaller pair (b, a₁): same b, a-coord descends via single τ-step.
    have hLT : Prod.Lex
        (Relation.TransGen (fun s' s => lts.Tr s Cslib.HasTau.τ s'))
        (Relation.TransGen (fun s' s => lts.Tr s Cslib.HasTau.τ s'))
        (b, a₁) (b, a) :=
      Prod.Lex.right b (Relation.TransGen.single hFirst)
    have htail : Relation.ReflTransGen R a₁ b :=
      ih (b, a₁) hLT b a₁ rfl hTail hπa₁ hπb
    -- Prepend the (now inert) head step.
    exact Relation.ReflTransGen.head ⟨hFirst, hπa, hπa₁⟩ htail
  -- CASE B (Or.inr): matched τ-prefix `b →τ*→ t →τ→ t'` with `r a t`, `r a₁ t'`.
  -- We have ⟦t⟧ = ⟦a⟧ = B from `r a t`.
  have hπt : Quotient.mk (brSetoid lts) t = B :=
    (Quotient.sound (s := brSetoid lts) ⟨r, hrat, hBis⟩).symm.trans hπa
  -- Convert hSTr (STr along τ) to a ReflTransGen path.
  have hPathBT : Relation.ReflTransGen (Cslib.LTS.Tr.toRelation lts Cslib.HasTau.τ) b t :=
    (Cslib.LTS.sTr_τSTr lts).mp hSTr
  -- TransGen WF descent from b to t' (since `b →τ*→ t →τ→ t'` is at least one step).
  -- Build it via cases on the head of `hPathBT` plus an aux that follows the existing pattern.
  have hBT' : Relation.TransGen (fun s' s => lts.Tr s Cslib.HasTau.τ s') t' b := by
    rcases hPathBT.cases_head with hEq | ⟨b₁, hHead, hRest⟩
    · -- Empty: b = t. The single step `hTrt : Tr b τ t'` IS the chain.
      subst hEq
      exact Relation.TransGen.single hTrt
    · -- Non-empty: b →τ→ b₁ →τ*→ t →τ→ t'. Build TransGen for tail then prepend head.
      have aux : ∀ {z}, Relation.ReflTransGen
          (Cslib.LTS.Tr.toRelation lts Cslib.HasTau.τ) b₁ z →
          Relation.TransGen (fun s' s => lts.Tr s Cslib.HasTau.τ s') z b := by
        intro z h
        induction h with
        | refl => exact Relation.TransGen.single hHead
        | tail _ hLast IHaux => exact Relation.TransGen.head hLast IHaux
      -- (aux hRest) : TransGen (rev) t b. Prepend hTrt : Tr t τ t'.
      exact Relation.TransGen.head hTrt (aux hRest)
  -- Split on whether ⟦t'⟧ = B (matched step inert) or not.
  by_cases hπt' : Quotient.mk (brSetoid lts) t' = B
  · -- B-inert subcase: ⟦t'⟧ = B, so from `r a₁ t'` we get ⟦a₁⟧ = ⟦t'⟧ = B.
    have hπa₁ : Quotient.mk (brSetoid lts) a₁ = B :=
      (Quotient.sound (s := brSetoid lts) ⟨r, hra₁t', hBis⟩).trans hπt'
    -- Identical to Case A from here on: descend in a.
    have hLT : Prod.Lex
        (Relation.TransGen (fun s' s => lts.Tr s Cslib.HasTau.τ s'))
        (Relation.TransGen (fun s' s => lts.Tr s Cslib.HasTau.τ s'))
        (b, a₁) (b, a) :=
    ...
    have htail : Relation.ReflTransGen R a₁ b :=
      ih (b, a₁) hLT b a₁ rfl hTail hπa₁ hπb
    exact Relation.ReflTransGen.head ⟨hFirst, hπa, hπa₁⟩ htail
  · -- B-non-inert subcase: ⟦t'⟧ ≠ B, so the matched tail step `t →τ→ t'` is non-inert.
    --
    -- Context at this point:
    --   • a →τ→ a₁ →τ*→ b              (original path; `a→τ→a₁` may be non-inert)
    --   • b →τ*→ t →τ→ t'              (BB-matched τ-prefix on b's side)
    --   • r a b, r a t, r a₁ t'
    --   • ⟦a⟧ = ⟦b⟧ = ⟦t⟧ = B,  ⟦a₁⟧ = ⟦t'⟧ ≠ B
    --   • TransGen descent  t' <τ⁺ b   (via `hBT'`)
    --
    -- IH is available at any `(b', a') <lex (b, a)`, i.e., at any pair where either the first
    -- coordinate τ-strictly descends from `b` or the second coordinate τ-strictly descends from
    -- `a` (with `b` unchanged). Both coordinates use `TransGen` so multi-step jumps are admitted.
    --
    -- The classical proof (van Glabbeek–Weijland 1996; Basten 1996) closes this case by
    -- iterating the backward BB-match along the entire path `a₁ →τ*→ b →τ→ t'` and tracking
    -- how block boundaries are crossed. Concretely:
    --   • Apply IH at `(t', a₁)` on the composite path `a₁ →τ*→ b →τ→ t'`. This is a same-block
    --     path (both endpoints in `⟦a₁⟧ = ⟦t'⟧ ≠ B`), and `(t', a₁) <lex (b, a)` via the first
    --     coordinate (`hBT'`). IH yields `ReflTransGen R_{⟦a₁⟧} a₁ t'`.
    --   • Backward-match `t →τ→ t'` along `r a t` to produce a witness path `a →τ*→ a_p →τ→ a_p'`
    --     with `r a_p t` (so `⟦a_p⟧ = B`) and `r a_p' t'` (so `⟦a_p'⟧ ≠ B`).
    --
    -- The remaining step is bridging from these two pieces back to `ReflTransGen R_B a b`,
    -- which we have not yet formalised. The conceptual obstacle is that the IH at `(t', a₁)`
    -- produces an inert path in block `⟦a₁⟧ ≠ B`, so it cannot directly be composed into the
    -- goal in block `B`. Closing this likely requires an additional combinator (e.g. a stutter-
    -- backed "canonical inert path" construction) that lives outside the present Prod.lex IH.
    --
    -- The restructure makes this gap visible and modular: a single, well-located `sorry` at
    -- the genuinely-hard subcase, replacing the previous nested cascade.
    sorry
